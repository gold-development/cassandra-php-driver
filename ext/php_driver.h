...
// Wrapper for zend_hash_sort to adapt qsort-style compare functions
static int
php_driver_bucket_compare(Bucket* a, Bucket* b, compare_func_t cmp)
{
...
#define PHP5TO7_ZEND_HASH_SORT(ht, compare_func, renumber) \
  zend_hash_sort(ht, (bucket_compare_func_t) php_driver_bucket_compare, renumber TSRMLS_CC)
...
#define PHP5TO7_ZVAL_COPY(zv1, zv2) \
  do {                              \
    zv1 = zv2;                      \
    if (zv1)                        \
      Z_TRY_ADDREF_P(zv1);          \
